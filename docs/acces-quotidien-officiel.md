...
4. précise que l'information peut changer chaque jour selon les consignes officielles.
...
Une interdiction locale communale absente de ces sources centrales peut donc ne pas être couverte. Dans ce cas, l'agent dit qu'il ne peut pas confirmer l'accès ; il ne déclare jamais le lieu ouvert par défaut.
...
Les accents, apostrophes et espaces sont normalisés pour la recherche, mais le nom officiel est conservé pour la réponse. Si un nom correspond à plusieurs juridictions, l'agent demande uniquement la commune ou la province.
